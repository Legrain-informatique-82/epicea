...
#pragma resource "*.dfm"
TInstForm *InstForm;
System::AnsiString EXEName, EXETitle;
 ...
 */
void __fastcall TInstForm::MakeAssociation( System::AnsiString Ext, System::AnsiString PgmToLinkTo ) {
...
//---------------------------------------------------------------------------
void __fastcall TInstForm::RemoveAssociation( System::AnsiString Ext ) {
	Ext = LowerCase( Ext );
...
//---------------------------------------------------------------------------
void __fastcall TInstForm::RegDeleteKey( HKEY RKey, System::AnsiString KeyPath ) {
...
void __fastcall TInstForm::SetValInReg( HKEY RKey, System::AnsiString KeyPath, System::AnsiString ValName, System::AnsiString NewVal ) {
	TRegistry *rg = new TRegistry();
 ...
 */
HRESULT __fastcall TInstForm::MakeLink( System::AnsiString PgmPath, System::AnsiString PgmArgs,
		...
		if ( hRes == S_OK ) {
			...
			if ( SUCCEEDED( hRes ) ) {
				...
				if ( SUCCEEDED( hRes ) ) {
					...
					hRes = PFile->Save( WFileName, false );
					PFile->Release();
				}
				ShLink->Release();
			}
...
void __fastcall TInstForm::UnInstButClick( TObject *Sender ) {
	...
	if ( StartMenuCB->Checked ) {
		GetSpecialFolder( (StartMenuRB->Checked) ? CSIDL_STARTMENU : CSIDL_PROGRAMS, path );
		System::AnsiString Temp = path + EXETitle + ".lnk";
 ...
 */
long __fastcall TInstForm::GetSpecialFolder( int aFolder, System::AnsiString &Location ) {
