...
{$IFNDEF DELPHI2}
  {$IFNDEF DELPHI3}
    Error! You must define a symbol according to your Delphi Version !
  {$ENDIF}
{$ENDIF}
...
uses
  ...
  Extctrls, Filectrl; { PrgForm is the progress bar form }
...
// PB le 23/10/03    FProgressForm   : TProgform;
  ...
  public
    ...
    procedure Start;                                  { Start copy of multiple files, so we'll create the Progress form only once}
    procedure Finish;                                 { Finish the copy of multiple files, so we'll destroy the Progress form}
...
procedure Register;
begin
  RegisterComponents('MMM Misc', [TCopyFile]);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Source<0 then
        {$IFDEF DELPHI3}
          raise EFOpenError.CreateFmt('Erreur ouverture fichier', [FCopyTo]);
        {$ELSE}
          raise EFOpenError.CreateFmt(LoadStr(SFCreateError), [FCopyTo]);
        {$ENDIF}
      try
        Dest := FileCreate(FCopyTo);
        if Dest<0 then
        {$ifdef DELPHI3}
          raise EFCreateError.CreateFmt('Erreur ouverture fichier',[FCopyTo]);
        {$else}
          raise EFCreateError.CreateFmt(LoadStr(SFCreateError),[FCopyTo]);
        {$endif}
        try
          repeat
...
// PB le 23/10/03      if progress and not Assigned(FProgressForm) then
// PB le 23/10/03        begin
// PB le 23/10/03          FProgressForm := TProgForm.Create(Application);
// PB le 23/10/03          FProgressForm.Caption := FCaption;
// PB le 23/10/03          FProgressForm.Show;
// PB le 23/10/03        end;

      if SendEvents and Assigned(FOnStartOperation) then
        FOnStartOperation( Self );
    end;
// PB le 23/10/03  else if Assigned(FProgressForm) then
// PB le 23/10/03    FProgressForm.Caption := FCaption;
end;
...
// PB le 23/10/03      if Assigned(FProgressForm) then
// PB le 23/10/03        begin
// PB le 23/10/03          FProgressForm.Free;
// PB le 23/10/03          FProgressForm := nil;
// PB le 23/10/03        end;
    end;
end;
...
// PB le 23/10/03  if Assigned(FProgressForm) then
// PB le 23/10/03    begin
// PB le 23/10/03      if FShowFileNames then
// PB le 23/10/03        FProgressForm.Lfilename.Caption := filename
// PB le 23/10/03      else
// PB le 23/10/03        FProgressForm.Lfilename.Caption := '';
// PB le 23/10/03      FProgressForm.Lfilename.Update;
// PB le 23/10/03      Application.ProcessMessages;
// PB le 23/10/03    end;
...
begin
  Start;
// PB le 23/10/03  if Assigned(FProgressForm) then
// PB le 23/10/03    FProgressForm.ProgressBar1.Visible := False;
  try
    result := DoCalcSize( RemoveSlash(dir), '' );
  finally
// PB le 23/10/03    if Assigned(FProgressForm) then
// PB le 23/10/03      FProgressForm.ProgressBar1.Visible := True;
    Finish;
  end;
end;
