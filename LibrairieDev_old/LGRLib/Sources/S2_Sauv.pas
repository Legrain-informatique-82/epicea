...
uses
  ...
  libLocale,
  UMail,
  CopyFile, MyBackup,ShellAPI, AppEvnts, ZipMstr19;
...
NomFichierSauvegarde,NomFichierSauvegarde2,NomFichierSauvegardeTmp,NomFicControle:String;
...
for i:=0 to ListeDesNomsdeDossier.Count-1 do
 ...
 //showmessage('ListeDesNomsdeDossier.count : '+inttostr(ListeDesNomsdeDossier.Count));
   ...
   else NomFichierSauvegarde:=ListeDesNomsdeDossier.Strings[i]+ListeTexteNomSauvegarde.Strings[i]+'.zlg';
   NomFichierSauvegarde2:=ListeDesNomsdeDossier.Strings[i]+Jour+Mois+DateInfosLgr(now).AnStr+'-'+Heure+ListeTexteNomSauvegarde.Strings[i]+'.zlg';
...
//        retourShell:=shellExecute(handle,'Open',Pchar(ExtractFilePath(Application.ExeName)+'7za.exe'),Pchar(cheminCmd),nil,SW_SHOWNORMAL);
      ...
      ////retourShell:=ShellExecute(handle,'Open',Pchar(ExtractFilePath(Application.ExeName)+'7za.exe'),Pchar(cheminCmd),nil,SW_SHOWNORMAL);
       //showmessage(inttostr(retourShell));
      Add;
      //Showmessage('Après add : '+IntToStr(r));
//      Showmessage(IntToStr(j));
...
//      showmessage(ListeRepertoireDEstination2.Strings[i][1]);
          ...
          MyBackup1.Backup;
//          showmessage('après Backup rep 2');
         end
        else
         begin
          if CopyAllFiles(ZipMaster1.ZipFilename,ListeRepertoireDEstination2.Strings[i]+'\'+NomFichierSauvegarde2,False) then
...
//  if( retourShell<=32)then
 ...
 // ShowMessage(cheminFichierlog);
  AjouterFichierTexte(cheminFichierlog,'Le '+Jour+'-'+Mois+'-'+DateInfosLgr(now).AnStr+' à '+H+'-'+M+'-'+S+
