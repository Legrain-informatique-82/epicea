...
type
  ...
  public
    ...
    ListeParamRestauration:TStringList;
    AControler:Boolean;
         ...
         begin
              ...
              if ListLecteur.IndexOf(ExtractFileDrive(ListeFichierARestaurer.Strings[i])+'\') <> -1 then
               begin
                ...
                CheminSansLecteur := IncludeTrailingPathDelimiter(ExtractFilePath(ListeFichierARestaurer.Strings[i]))+ '*.zlg';
...
if ParamCount = 1 then
 begin
  if FileExists(ParamStr(1)) then
       ListeParamRestauration.LoadFromFile(ParamStr(1))
       else
        begin
...
//    ListeEMail.Add(str_getstringelement(ListeParamSauvegarde.Strings[i],5,';'));
 ...
 begin
    BtnRestaure.Enabled := (paramCount >= 3);
    ...
    ListeNomDeDossier.Add(ParamStr(1));
    NomDossier:=ParamStr(1);
...
//      MessageDlg('Le format du fichier n''est pas correct !', mtError, [mbOK], 0);
    ...
    ListeRepertoireCourantDestination.Add(ParamStr(2));
    RepDossier:=ParamStr(2);
...
//      MessageDlg('Le format du fichier n''est pas correct !', mtError, [mbOK], 0);
    ...
    ListeFichierARestaurer.Add(ParamStr(3));
    FichierZip:=ParamStr(3);
