...
uses
  ...
  ExtCtrls,stdctrls, Panel_PM,DBGrids,DBTables,db,Edit_PM,menus,Variants;

type
  TResultatEvent = Procedure(Sender :TObject; Valide :Boolean) of object;
  TTypeTri = (Croissant,Decroissant);
  TPanelAide_PM = class(TPanel_PM)
  private
...
procedure Register;
begin
  RegisterComponents('PBMA', [TPanelAide_PM]);
end;

constructor TPanelAide_PM.Create(Aowner: TComponent);
begin
...
destructor TPanelAide_PM.Destroy;
begin
     inherited;
end;

function  TPanelAide_PM.requete:string;
begin
case TriSQL of
    croissant:Tri:=' ASC';
...
function  TPanelAide_PM.requeteLike:string;
var
LikeText:string;
begin
case TriSQL of
...
procedure TPanelAide_PM.loaded;
var
...
procedure TPanelAide_PM.KeyPressFEdit(Sender: TObject; var Key: Char);
var
I,J,K:Integer;
aux,auxtmp:string;
ValeurChCle:Variant;
...
Procedure TPanelAide_PM.EnterFEdit(sender:TObject);
var
i:integer;
begin
...
procedure TPanelAide_PM.DblClick(sender:TObject);
var
key:char;
begin
key:=#13;
...
Procedure TPanelAide_PM.KeyDownFEdit(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
...
Procedure TPanelAide_PM.ChangeFEdit(sender:TObject);
var
NomChamp:string;
begin
...
procedure TPanelAide_PM.BeforeScroll(DataSet: TDataSet);
begin
FEditHelp.OnChange:=nil;
end;

procedure TPanelAide_PM.AfterScroll(DataSet: TDataSet);
begin
if FEditHelp.Text <>'' then
   begin
   if FEditHelp.Text[1]<>'*' then
...
Procedure TPanelAide_PM.ColEnter(sender:TObject);
begin
...
Procedure TPanelAide_PM.ColExit(sender:TObject);
begin
   TmpQuery.BeforeScroll:=nil;
   TmpQuery.AfterScroll:=nil;
end;

procedure TPanelAide_PM.DelayTimer(Sender: TObject);
begin
  FEditHelp.OnChange:=ChangeFEdit;
...
Procedure TPanelAide_PM.ExitFEdit(sender:TObject);
begin
...
Procedure TPanelAide_PM.ExitFGrid(sender:TObject);
begin
Colexit(FGridHelp);
...
Function TPanelAide_PM.GetStringElement(aString :String; aPos :Integer; cSep :Char) :String;
