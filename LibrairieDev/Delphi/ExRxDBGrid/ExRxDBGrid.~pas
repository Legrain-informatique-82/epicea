...
type
    ...
    private
        SignalColEnter:Boolean; // cette varible est à true quand le colenter viens de se faire
                                 // elle sert pour le signaler au cellClick a cause des boolean
                                 //  qui change directement de valeur
        SignalRowEnter:boolean;
...
procedure TExRxDBGrid.MnuShowMemoEllipsisClick(Sender: TObject);
begin
    if FExRxDBGridPopUpMenu.Items[7].Checked = False then
        begin
            Datalink.DataSet.DisableControls;
            TExRxDBGrid(Self).ShowMemoEllipsis := True;
            Datalink.DataSet.EnableControls;
            FExRxDBGridPopUpMenu.Items[7].Checked := True;
        end
    else
        begin
            Datalink.DataSet.DisableControls;
            TExRxDBGrid(Self).ShowMemoEllipsis := False;
            Datalink.DataSet.EnableControls;
            FExRxDBGridPopUpMenu.Items[7].Checked := False;
        end;
end;
...
begin
    if FExRxDBGridPopUpMenu.Items[11].Checked = False then
        begin
            Datalink.DataSet.DisableControls;
            TExRxDBGrid(Self).CellHints := True;
            Datalink.DataSet.EnableControls;
            FExRxDBGridPopUpMenu.Items[11].Checked := True;
        end
    else
        begin
            Datalink.DataSet.DisableControls;
            ...
            Datalink.DataSet.EnableControls;
            FExRxDBGridPopUpMenu.Items[11].Checked := False;
        end;
end;
...
begin
    ...
    FExRxDBGridPopUpMenu.Items[7].Checked := TExRxDBGrid(Self).ShowMemoEllipsis;
    FExRxDBGridPopUpMenu.Items[8].Checked := TExRxDBGrid(Self).FullSizeMemo;
    FExRxDBGridPopUpMenu.Items[10].Checked := TExRxDBGrid(Self).RowSizingAllowed;
    FExRxDBGridPopUpMenu.Items[11].Checked := TExRxDBGrid(Self).CellHints;
    FExRxDBGridPopUpMenu.Items[12].Checked := TExRxDBGrid(Self).MultiLineTitles;
    FExRxDBGridPopUpMenu.Items[13].Checked := TExRxDBGrid(Self).UseRowColors;
end;
...
procedure TExRxDBGrid.SetShowMemoEllipsis(Value: Boolean);
begin
...
begin
    ...
    if (DataLink.Active) and (LogCol >= 0) and (LogRow >= 0) then
        begin
            ...
            try
                ...
                if CurrencyAffich then
                 begin
                   Text :=Str_QueDesChiffres(text);
                   if not str_empty(text) then
                    begin
                     text:=FormatFloat(',0.00 F;-,0.00 F;,0.00 F', (StrToCurr(text) * 6.55957));
                    end;
                 end;
            finally
    ...
    if (Text <> '') and not EditorMode and
        ((Canvas.TextWidth(Text) + TextOffset > ColWidths[Col]) or (CurrencyAffich)) and
...
begin
NewRow:=row;
SignalRowEnter:=(LastRow <> NewRow);
    if not (csDesigning in ComponentState) then
        begin
            if not ReadOnly and Datalink.Active and not Datalink.Readonly then
                begin
                    BeginUpdate;
                    if ((not SignalColEnter) and (not SignalRowEnter)) then
                     begin
                    ...
                    LastRow := NewRow;
                    SignalColEnter:=false;
                    SignalRowEnter:=false;
