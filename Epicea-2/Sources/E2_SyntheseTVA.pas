...
//      totauxbalance:=DMDiocEtatBal.SommeTotauxBalanceParCompte(e.DatExoDebut-1,SyntheseTVA.Periode.DateFin,DM_C_CompteTVADeduiteAReverser);
  ...
  // GrGrid2 : TVA DEDUCTIBLE
   ...
   // grgrid5 : tva autoliquidée

    for k:=0 to GrGrid5.rowcount-1 do
      GrGrid5.rows[k].clear;
    GrGrid5.RowCount := 2;
    GrGrid5.RowHeights[0] := 0;
    GrGrid5.ParamColor:=true;
    GrGrid5.Param:=ParamUtil.CouleurDBGrid;
    GrGrid5.DessineContourLigne:=false;
    GrGrid5.Cells[0,0] := '';
    GrGrid5.Cells[0,1] := 'Tva Autoliquidée sur importations';
    //créer et remplir la liste des codes tva correspondant aux tva Autoliquidée
    ListeObjet:=TStringList.Create;
    ListeObjet.Add(DM_C_CodeTVADefautAutoLiquide+';0');
    GrGrid5.Objects[0,1]:=ListeObjet;
    ...
    GrGrid5.Cells[1,1] := FormatFloat(E.FormatMonnaie + ';; ', TotAutoLiquideL1);
    GrGrid5.Cells[2,1] := FormatFloat(E.FormatMonnaie + ';; ', TotTVAautoLiquideL1);
  ...
  // GrGrid3 : OPERATIONS NON IMPOSABLES
    ...
    for i:=0 to ListeCodeTVATmp.Count-1 do
       begin
          ...
          StrTmp:=Copy(ListeCodeTVATmp.Strings[i],1,1);
          if(strTmp='F') and (ADesVentesEnFranchise=false)then ADesVentesEnFranchise:=true;
       end;
    DeFiltrageDataSet(DMREch.TaTvaCodeRech);
    ListeObjet:=TStringList.Create;
    ListeObjet.Assign(ListeCodeTVATmp);
...
//             TotTVADeductibleL2:=TotTVADeductibleL2+Abs(DMTmTVA.QuDec_Tva_VisuListe.findField('TottvaMontantDebit').AsCurrency - DMTmTVA.QuDec_Tva_VisuListe.findField('TottvaMontantCredit').AsCurrency);
    ...
    if(TotOpNonImposableL3Bis<>0)then
    begin
    GrGrid3.Cells[1,4] := FormatFloat(E.FormatMonnaie + ';; ', TotOpNonImposableL3Bis);
    GrGrid3.Cells[1,5] := FormatFloat(E.FormatMonnaie + ';; ', TotOpNonImposableL4);
    end
    else
    begin
