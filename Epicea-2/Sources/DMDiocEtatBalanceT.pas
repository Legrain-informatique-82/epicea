...
type
  ...
  TDMDiocEtatBalT = class(TDataModule)
    ...
    QuEcritureBase2Tiers: TStringField;
    QuEcritureBase2TotDebit: TCurrencyField;
    QuEcritureBase2TotCredit: TCurrencyField;
    ...
    QuEcritureBase2Compte: TStringField;
    QuEcritureBaseCorrection: TQuery;
    ...
    procedure TaBalanceaffichTFilterRecord(DataSet: TDataSet;
      var Accept: Boolean);
      procedure rechercheIndex(table:TTable);
  ...
  Function EditionBalanceTiersL(TypeEdition : integer; DemandePeriode:Boolean; ListeCompte:TStringList;DataSet:TDataSet;SautDePage:boolean;avecAdresse:boolean=false):Boolean;
...
try
 try
    ...
    if DMRech=nil then DMRech:=TDMRech.create(Application.MainForm);
      ...
      DMDiocEtatBalT.QuEcritureBase2.Close;
      DMDiocEtatBalT.QuEcritureBase2.ParamByName('DateDeb').AsDate:=E.DatExoDebut;
      DMDiocEtatBalT.QuEcritureBase2.ParamByName('DateFin').AsDate:=E.DatExoFin;
      DMDiocEtatBalT.QuEcritureBase2.Open;
//    FiltrageDataSet(DMRech.TaBalanceTiersRech,'((Debit_Base <> 0 and Debit_Base <> Null) or (Credit_Base <> 0 and Credit_Base <> Null))');
    ...
    DMDiocEtatBalT.QuEcritureBase2.First;
    while not DMDiocEtatBalT.QuEcritureBase2.Eof do
     begin
     if (DMDiocEtatBalT.QuEcritureBase2.FindField('Tiers').AsString<>'') then
        begin//si tiers rempli
         if DMRech.TaBalanceTiersRech.Locate('Tiers',DMDiocEtatBalT.QuEcritureBase2.FindField('Tiers').AsString,[]) then
          begin
          DMDiocEtatBalT.QuCorrectionBalanceT.Close;
          DMDiocEtatBalT.QuCorrectionBalanceT.SQL.Clear;
          DMDiocEtatBalT.QuCorrectionBalanceT.SQL.add('update BalanceTiers.Db  set Debit_Base = '+str_QueDesChiffresCurr_SQL(DMDiocEtatBalT.QuEcritureBase2.FindField('TotDebit').AsString)+
                                                    ', Credit_Base = '+str_QueDesChiffresCurr_SQL(DMDiocEtatBalT.QuEcritureBase2.FindField('TotCredit').AsString)+
                                                    ' where Tiers='''+DMDiocEtatBalT.QuEcritureBase2.FindField('Tiers').AsString+'''');
        //  showmessage(DMDiocEtatBalT.QuCorrectionBalanceT.SQL.Text);
          DMDiocEtatBalT.QuCorrectionBalanceT.ExecSQL;
                 ListeMiseAJour.Add(DMRech.TaBalanceRech.FindField('Tiers').AsString+
                                    ' | '+
                                    DMDiocEtatBalT.QuEcritureBase2.FindField('Tiers').AsString+
                                    ' ## BaseD : '+
                                    DMRech.TaBalanceRech.FindField('Debit_Base').AsString+
                                    ' | '+
                                    DMDiocEtatBalT.QuEcritureBase2.FindField('Totdebit').AsString+
                                    ' ## BaseC : '+
                                    DMRech.TaBalanceRech.FindField('Credit_Base').AsString+
                                    ' | '+
                                    DMDiocEtatBalT.QuEcritureBase2.FindField('TotCredit').AsString
                                    );
           end
          else
          ...
          DMDiocEtatBalT.QuCorrectionBalanceT.SQL.add('insert into BalanceTiers.Db  (Tiers,compte,Debit_Report,Credit_Report,Debit_Base,Credit_base,Debit_Total,'+
                                                     ...
                                                     'values('''+DMDiocEtatBalT.QuEcritureBase2.FindField('Tiers').AsString+''','''+DMDiocEtatBalT.QuEcritureBase2.FindField('Compte').AsString+''',0,0,'+
                                                     str_QueDesChiffresCurr_SQL(DMDiocEtatBalT.QuEcritureBase2.FindField('TotDebit').AsString)+
                                                     ','+
                                                     str_QueDesChiffresCurr_SQL(DMDiocEtatBalT.QuEcritureBase2.FindField('TotCredit').AsString)+',0,0,0,0,0,0,0,0)');
//                                                     str_QueDesChiffresCurr_SQL(DMDiocEtatBalT.QuEcritureBase.FindField('TotCredit').AsString)+',0,0,0,0,0,0,0,0)');
      ...
      DMDiocEtatBalT.QuEcritureBase2.Next;
     end;
    except
     result := False;
    end;
...
if DMDiocEtatBalT=nil then DMDiocEtatBalT:=TDMDiocEtatBalT.Create(Application.MainForm);
if DMRech=nil then DMRech:=TDMRech.create(Application.MainForm);
  DMDiocEtatBalT.QuEcritureBaseCorrection.Close;
  DMDiocEtatBalT.QuEcritureBaseCorrection.ParamByName('DateDeb').AsDate:=E.DatExoDebut;
  DMDiocEtatBalT.QuEcritureBaseCorrection.ParamByName('DateFin').AsDate:=E.DatExoFin;
  DMDiocEtatBalT.QuEcritureBaseCorrection.Open;
  ListeTiersNonEquilibre.Add(DMDiocEtatBalT.QuEcritureBaseCorrection.sql.Text);
...
result := (DMDiocEtatBalT.QuEcritureBaseCorrection.RecordCount = DMRech.TaBalanceTiersRech.RecordCount);
...
if not result then
  result:=((DMDiocEtatBalT.QuEcritureBaseCorrection.RecordCount=1) and(VerifFNumeric(DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('Tiers')).VideOUZero)
  and(DMRech.TaBalanceTiersRech.RecordCount=0));
if not result then exit;

DMRech.TaBalanceTiersRech.First;
DMDiocEtatBalT.QuEcritureBaseCorrection.First;
while not DMDiocEtatBalT.QuEcritureBaseCorrection.Eof do
 begin
  Equilibre:=DMRech.TaBalanceTiersRech.Locate('Tiers',DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('Tiers').AsString,[]);
  //Equilibre:=(DMRech.TaBalanceTiersRech.FindField('Tiers').AsString = DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('Tiers').AsString);
  if Equilibre then
    Equilibre:=(DMRech.TaBalanceTiersRech.FindField('Debit_Base').AsCurrency = DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('TotDebit').AsCurrency);
  if Equilibre then
    Equilibre:=(DMRech.TaBalanceTiersRech.FindField('Credit_Base').AsCurrency = DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('TotCredit').AsCurrency);
   ...
   begin
     if ListeTiersNonEquilibre <> nil then
       ListeTiersNonEquilibre.Add(DMRech.TaBalanceTiersRech.FindField('Tiers').AsString+
                                ' | '+
                                DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('Tiers').AsString+
//                                ' ## ReportD : '+
//                                DMRech.TaBalanceRech.FindField('Debit_Report').AsString+
//                                ' | '+
//                                DMDiocEtatBal.QuEcritureBaseCorrection.FindField('Debit_Report').AsString+
//                                ' ## ReportC : '+
//                                DMRech.TaBalanceRech.FindField('Credit_Report').AsString+
//                                ' | '+
//                                DMDiocEtatBal.QuEcritureBaseCorrection.FindField('Credit_Report').AsString+
                                ' ## BaseD : '+
                                DMRech.TaBalanceTiersRech.FindField('Debit_Base').AsString+
                                ' | '+
                                DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('Totdebit').AsString+
                                ' ## BaseC : '+
                                DMRech.TaBalanceTiersRech.FindField('Credit_Base').AsString+
                                ' | '+
                                DMDiocEtatBalT.QuEcritureBaseCorrection.FindField('TotCredit').AsString
                                );
     result:= false;
   end;
  DMRech.TaBalanceTiersRech.Next;
  DMDiocEtatBalT.QuEcritureBaseCorrection.Next;
 end;
finally
DeFiltrageDataSet(DMRech.TaBalanceTiersRech);
...
Function EditionBalanceTiersL(TypeEdition : integer; DemandePeriode:Boolean; ListeCompte:TStringList;DataSet:TDataSet;SautDePage:boolean;avecAdresse:boolean):Boolean;
  var NomFich,Chemin:String;
  var periode:TPeriode;
...
ligneEdition,titreEnPlus:string;
Begin
try
...
//      cred_solde:=0;
      while not DMDiocEtatBalT.TaBalanceCalcT.Eof do
       begin
        ...
        ligneEdition:=GereLibelle(DMDiocEtatBalT.TaBalanceCalcT.FindField('Tiers').AsString)+
                         ';'+
         ...
         if(avecAdresse) and (DMRech.TaTiersRech.Locate('tiers',DMDiocEtatBalT.TaBalanceCalcT.FindField('Tiers').AsString,[]))then
         begin
           ligneEdition:=ligneEdition+';'+DMRech.TaTiersRech.findfield('type').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('actif').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('adresse1').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('adresse2').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('poste').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('ville').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('pays').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('livr_adr1').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('livr_adr2').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('livr_poste').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('livr_ville').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('livr_pays').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('telephone1').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('telephone2').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('telecopie').AsString+
                                      ';'+DMRech.TaTiersRech.findfield('mel').AsString ;
         end;

         listeEdition.Add( ligneEdition);
...
//         ListeValeur.AddStrings(LigneEdition);
     ...
     if(avecAdresse)then titreEnPlus:=';type;actif;adresse1;adresse2;poste;ville;pays;adresse liv. 1; adresse liv. 2;poste liv. ;ville liv. ; pays liv.;téléphone 1; téléphone 2;télécopie; email';
     ListeEdition.Insert(0,'Tiers;Compte;Désignation;Solde_Debit_Report;Solde_Credit_Report;Mouv_Debit;Mouv_Credit;Solde_Debit_Total;Solde_Credit_Total'+titreEnPlus);
...
QuEcritureBase2.ParamByName('DateDeb').AsDate:=DateDeb;
QuEcritureBase2.ParamByName('DateFin').AsDate:=DateFin;
    if QuEcritureBase2.Active then
     QuEcritureBase2.Refresh else QuEcritureBase2.Open;

if(TaBalanceaffichT.IndexDefs.IndexOf('Tiers')<>-1)then
  TaBalanceaffichT.IndexName:='Tiers'
else
    TaBalanceaffichT.IndexName:='';
...
begin
OuvreTables('',[TaPlanCpt,QuEcritureReport,QuEcritureBase2]);
if not QuEcritureBase2.Active  then QuEcritureBase2.Open;
...
//  begin
    ...
    if ForcerClose then QuEcritureBase2.Close;
//  end
//else
//  begin
//    QuEcritureReport.Close;
...
procedure TDMDiocEtatBalT.rechercheIndex(table:TTable);
var
i:integer;
begin
   for i:=0 to table.IndexDefs.Count-1 do
   begin
     if LowerCase(table.IndexDefs.Items[i].Name)='tiers_' then
