...
//  TFormNotCloseQuery = set of (Main);
  TMain = class(TForm)
    ...
    BalanceCerfaLiasseAsc: TMenuItem;
    ...
    LabDossier: TLabel;
    Exportpourcontrlfiscal1: TMenuItem;
    Panel8: TPanel;
    ...
    Panel10: TPanel;
    Panel9: TPanel;
    ImaSauvegardeInactive: TImage;
    Panel11: TPanel;
    BtnProprietesSauvegardeAuto: TSpeedButton;
    imaSauvegardeActive: TImage;
    ImageSauvegardeCloud: TImage;
    Gestiondescomptesbancaires1: TMenuItem;
    ImportationdesrelevsEbics1: TMenuItem;
    Crationdesrglements1: TMenuItem;
    EcrituressansTVA1: TMenuItem;
    Balancedestiersetleuradresse1: TMenuItem;
    ImportationCAISSE: TMenuItem;
    historiquedesequilibremouvement1: TMenuItem;
    ImportationCAISSE2: TMenuItem;
    Importationcaisse1: TMenuItem;
    Importationcaissebis1: TMenuItem;
...
//    procedure mnAcquisExerciceClick(Sender: TObject);
    ...
    Function OuvreExo(NomDossier,NomExo : String;Reindexation:boolean=true;sauvegarde:boolean=false):Boolean;
...
//    procedure Subventions4Click(Sender: TObject);
    ...
    procedure BalanceCerfaLiasseAscClick(Sender: TObject);
    procedure Button6Click(Sender: TObject);
    procedure Exportpourcontrlfiscal1Click(Sender: TObject);
//    procedure BalanceCerfaLiasseAsc1Click(Sender: TObject);
//    procedure mnChoixAmortisClick(Sender: TObject);

    procedure afficheParamSauvegardeAuto;
    procedure BtnProprietesSauvegardeAutoClick(Sender: TObject);
    procedure ImportationdesrelevsEbics1Click(Sender: TObject);
    procedure Gestiondescomptesbancaires1Click(Sender: TObject);
    procedure Crationdesrglements1Click(Sender: TObject);
    procedure Balancedestiersetleuradresse1Click(Sender: TObject);
    procedure historiquedesequilibremouvement1Click(Sender: TObject);
    procedure Importationcaissebis1Click(Sender: TObject);
    procedure ImportationCAISSE2Click(Sender: TObject);
    procedure ImportationCAISSEClick(Sender: TObject);
  private
...
//, E2_RelanceTiers
  ...
  E2_ChangementTauxTva,TStringlistLGR, InputQueryFrm_F, ListeModules,
  ImportReleveEbics, ComptesBancaires, CreationReglementReleve,
  ChoixParametresFEC;
...
Function TMain.OuvreExo(NomDossier,NomExo : String;Reindexation:boolean;sauvegarde:boolean):Boolean;
begin
 try
    try
    result:=false;
      if FermerDossierSauvegarde(E,sauvegarde) then
//      if DetruitTout then
      begin
        if (DMPlan = nil) then
...
try
   ...
   if(e.NomDossier<>NomDossier)then
   begin
      //SauvegardeFermetureAuto(true,e.AutoriserSauvegardeFermeture,e.RepertoireSauvegardeFermture);
   end;
   if (((sender as TMenuItem).Name='Exo0') or ((sender as TMenuItem).Name='Exo1')) then
    begin
     if OuvreExo(NomDossier,NomExo,true,e.NomDossier<>NomDossier) then
...
begin
 CorrectionIdEcriture(nil,false);
SauvegardeFermetureAuto(e.AfficherMessageSauvegardeFermeture,e.AutoriserSauvegardeFermeture,e.RepertoireSauvegardeFermture);
                                               //,Convertisseur
...
Panel4Resize(Panel4);
afficheParamSauvegardeAuto;
//WebBrowser2.Visible:=false;
end;

procedure TMain.afficheParamSauvegardeAuto;
begin
imaSauvegardeInactive.Visible:=not e.AutoriserSauvegardeFermeture ;
imaSauvegardeActive.Visible:=e.AutoriserSauvegardeFermeture  and not trouveFichierCloud;
ImageSauvegardeCloud.Visible:=e.AutoriserSauvegardeFermeture and trouveFichierCloud;
end;
...
begin
FormateTailleBoutton([BtnPlanCpt,BtnPlanAux,BtnPointages,BtnSaisiePiece,BtnFermer,BtnBalance],PaBtn,110,0);
end;
...
Ecran_autorisationGenerale,
Ecran_Autorise_TypeClient1,
Ecran_Autorise_TypeVersionMaints:Tstringlist;
ExoN1:boolean;
var
...
//  if(FileExists(IncludeTrailingPathDelimiter(e.RepertoireDossier)+'autorisation.db'))then
//  begin
//      DMGestVersion.EcritAutorisationDansDossierLocal();
//      liste:=TListeModuleLGR.Create;
//     liste:=RecupAutorisation;
//      if(not empty(liste.fichier))then
//          liste.listeModule:= liste.retourneListeAutorisationsLocale(liste.fichier);
//
//
//      MenuTous:=RecupListeMenuTous;
//      MenuTroncCommun:=RecupListeMenuTroncCommun;
...
//      Ecran_Autorise_TypeVersionMaints:=RecupListeInclusion('Maints');
//
//
//      //on ferme tous les menus décrit dans la table gestVersion
//      for iMain:=0 to MenuTous.count-1 do
//          VisibleItemMenu([TMenuItem(FindComponent(MenuTous.Strings[iMain]))],false);
//
//      //on ouvre tous les menus qui ne sont pas variable, donc qui sont dans tronc commun
//      for iMain:=0 to MenuTroncCommun.count-1 do
...
//          VisibleItemMenu([TMenuItem(FindComponent(Ecran_A_Verrouille_Toujours.Strings[iMain]))],false);
//
//      // on ouvre tous les menus contenu dans la liste des modules autorisés
//
//      for iMain:=0 to liste.listeModule.count-1 do
//          VisibleItemMenu([TMenuItem(FindComponent(liste.listeModule.Strings[iMain]))],true);
//
//      //on ferme tous typeutilisateur ou typeDossier (général,ou autre) menu à verrouiller
//      for iMain:=0 to Ecran_A_Verrouille_TypeClient.count-1 do
//          VisibleItemMenu_SiDejaVisible([TMenuItem(FindComponent(Ecran_A_Verrouille_TypeClient.Strings[iMain]))],false);
//
//        //on ouvre tous les  typeClient menu autorises
...
//   else
   ...
   //showmessage('ok');
     ...
     // Ecran_A_Verrouille_TypeDossier:=RecupListeExclusionMenu_Type(e.TypeDossierToStr(e.TypeDossier));
      ...
      for iMain:=0 to Ecran_A_Verrouille_Toujours.count-1 do
      begin    //
          if(Ecran_Autorise_TypeVersionMaints.IndexOf(Ecran_A_Verrouille_Toujours.Strings[iMain])<>-1)then
            VisibleItemMenu([TMenuItem(FindComponent(Ecran_A_Verrouille_Toujours.Strings[iMain]))],((ParamUtil.maintenance) and (ParamUtil.TypeMaintenance='MaintS')))
          else
            VisibleItemMenu([TMenuItem(FindComponent(Ecran_A_Verrouille_Toujours.Strings[iMain]))],false);
      end;
      //on traite typeClient menu à verrouiller
...
//  Liassefiscale2.Visible:=(E.TypeClient = CL_Cogere);
  ...
  for iMain:=0 to Ecran_Autorise_TypeVersionMaints.count-1 do
      VisibleItemMenu_SiDejaVisible([TMenuItem(FindComponent(Ecran_Autorise_TypeVersionMaints.Strings[iMain]))],((ParamUtil.maintenance) and (ParamUtil.TypeMaintenance='MaintS')));
...
if ListeCpt.Result then
  EditionDetteCreanceL_affinee(C_EditionSimple,True,ListeCpt.Liste,nil,ListeCpt.SautDePage,false);
finally
end;
...
ListeCpt:=SelectionListeComptePourDettesEtCreances(ParamAffichCompte);
if ListeCpt.Result then
  EditionDetteCreanceL_affinee(C_EditionSimple,True,ListeCpt.Liste,nil,ListeCpt.SautDePage,false,true);
finally
end;
end;
...
ListeCpt:=SelectionListeComptePourDettesEtCreances(ParamAffichCompte);
if ListeCpt.Result then
  EditionDetteCreanceL_affineeEcheance(C_EditionSimple,True,ListeCpt.Liste,nil,ListeCpt.SautDePage,false,true);
finally
end;
end;
...
if ((DMDiocEtatBal.TaBalanceaffich.Filtered) and ((not empty(DMDiocEtatBal.TaBalanceaffich.Filter)or(DMDiocEtatBal.ListeFiltreAffich.count<>0)))) then
 begin
   if MessageDlg('La balance est filtrée, vous risquez de ne pas exporter toutes '+#13+#10+'les informations !'+#13+#10+''+#13+#10+'Êtes vous sûr de vouloir continuer ?', mtWarning, [mbYes,mbNo], 0) = mrYes then
      //case e.TypeClient of
         //cl_cogere:
         ExportBalanceCOGEREFiscal_Liasse(DMDiocEtatBal.TaBalanceaffich);
      //end;//fin du case
 end
 else
  begin
...
begin
  try
      ...
      TableGereCommit(DMRech.TaBalanceRech);
      TableGereCommit(DMRech.TaPieceRech);
...
if(not DirExists(Dirtomcat+'\webapps\birt\report'))then
begin
...
//    requete.SQL.Add(filtre);
    ...
    ListeCpt.SautDePage:=ParamAffichCompte.sautDePage;
    result:=ListeCpt;
    Requete.Close;
    requete.free;
end;
...
procedure TMain.BalanceCerfaLiasseAscClick(Sender: TObject);
begin
if ((DMDiocEtatBal.TaBalanceaffich.Filtered) and ((not empty(DMDiocEtatBal.TaBalanceaffich.Filter)or(DMDiocEtatBal.ListeFiltreAffich.count<>0)))) then
 begin
   if MessageDlg('La balance est filtrée, vous risquez de ne pas exporter toutes '+#13+#10+'les informations !'+#13+#10+''+#13+#10+'Êtes vous sûr de vouloir continuer ?', mtWarning, [mbYes,mbNo], 0) = mrYes then
      //case e.TypeClient of
         //cl_cogere:
         ExportBalanceCOGEREFiscal_LiasseAsc(DMDiocEtatBal.TaBalanceaffich);
      //end;//fin du case
 end
 else
  begin
...
begin
 liste:=TStringlistLGR1.Create;
 Query:=TQuery.Create(self);
 Query.DatabaseName:=DM_C_NomAliasDossier;
 Query.SQL.Text:='select * from plancpt';
 Query.Open;
 Query.first;
 while(not Query.Eof)do
 begin
//   if(Query.FindField('Compte')<>nil)then valeur='''+Query.FindField('Compte').asstring;
//   ligne:='AppendRecord(['''+Query.FindField('Compte').AsVariant+
//   ''','''+Query.FindField('Libelle').AsString+
//   ''','+FloatToStr_Lgr(Query.FindField('Debit_Deb').AsFloat)+
//   ','+FloatToStr_Lgr(Query.FindField('Credit_Deb').AsFloat)+
//   ','''+Query.FindField('Un1').AsString+
//   ''','''+Query.FindField('Un2').AsString+
//   ''','+FloatToStr_Lgr(Query.FindField('Qt1_Deb').AsFloat)+
//   ','+FloatToStr_Lgr(Query.FindField('Qt2_Deb').AsFloat)+
//   ','+BoolToStrEnglish(Query.FindField('PlanSaisie').AsBoolean)+
//   ','''+Query.FindField('TvaCode').AsString+
//   ''','''+Query.FindField('TvaType').AsString+
//   ''','+BoolToStrEnglish(Query.FindField('TvaDebit').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('SensCredit').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Collectif').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Centraliser').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Tiers').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Rapprochement').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Immobilisation').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('ImmoAmort').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('ImmoDegressif').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Emprunt').AsBoolean)+
//   ','+BoolToStrEnglish(Query.FindField('Abandonne').AsBoolean)+
//   ','''+Query.FindField('Qui').AsString+
//   ''','''+Query.FindField('Quand').AsString+
//   ''','+IntToStr_Lgr(Query.FindField('ID_Affichage').AsInteger)+
//   ','''+Query.FindField('JBanque').AsString+
//   ''','+BoolToStr(Query.FindField('pointable').AsBoolean)+
//   ','+FloatToStr_Lgr(Query.FindField('taux').AsFloat)+
//   ','''+Query.FindField('DIV').AsString+
//   ''','+IntToStr_Lgr(Query.FindField('ID_GESTANAL').AsInteger)+
//   ','''+Query.FindField('CODE_GESTANAL').AsString+
//   ''','+BoolToStrEnglish(Query.FindField('UtilOuvAuto').AsVariant)+']);';
   liste.Add(ligne);
   ligne:='AppendRecord(['''+Query.FindField('Compte').AsString+
   ''','''+Query.FindField('Libelle').AsString+
   ''','+Query.FindField('Debit_Deb').AsString+
   ','+Query.FindField('Credit_Deb').AsString+
   ','''+Query.FindField('Un1').AsString+
   ''','''+Query.FindField('Un2').AsString+
   ''','+Query.FindField('Qt1_Deb').AsString+
   ','+Query.FindField('Qt2_Deb').AsString+
   ','+Query.FindField('PlanSaisie').AsString+
   ','''+Query.FindField('TvaCode').AsString+
   ''','''+Query.FindField('TvaType').AsString+
   ''','+Query.FindField('TvaDebit').AsString+
   ','+Query.FindField('SensCredit').AsString+
   ','+Query.FindField('Collectif').AsString+
   ','+Query.FindField('Centraliser').AsString+
   ','+Query.FindField('Tiers').AsString+
   ','+Query.FindField('Rapprochement').AsString+
   ','+Query.FindField('Immobilisation').AsString+
   ','+Query.FindField('ImmoAmort').AsString+
   ','+Query.FindField('ImmoDegressif').AsString+
   ','+Query.FindField('Emprunt').AsString+
   ','+Query.FindField('Abandonne').AsString+
   ','''+Query.FindField('Qui').AsString+
   ''','''+Query.FindField('Quand').AsString+
   ''','+Query.FindField('ID_Affichage').AsString+
   ','''+Query.FindField('JBanque').AsString+
   ''','+Query.FindField('pointable').AsString+
   ','+Query.FindField('taux').AsString+
   ','''+Query.FindField('DIV').AsString+
   ''','+Query.FindField('ID_GESTANAL').AsString+
   ','''+Query.FindField('CODE_GESTANAL').AsString+
   ''','+Query.FindField('UtilOuvAuto').AsString+']);';
   Query.Next;
 end;
 Query.Close;
   liste.SaveToFile(IncludeTrailingPathDelimiter(e.RepertoireDossier)+'AppendRecordPlanCpt.txt');
   Initialise_TStringlist(liste);
end;

procedure TMain.Exportpourcontrlfiscal1Click(Sender: TObject);
var
deb,fin:tdate;
siren,siret:string;
...
datefin:Tdate;
legende:Tlabel;
resultChoixParamFEC:TResultChoixParamFEC;
  begin
      resultChoixParamFEC:=ChoixParamFECAffiche;
       if(resultChoixParamFEC.Retour)then
       begin
      //Récupérer le numéro siren dans les infos juridique
        ...
        begin
            ...
            legende.Caption:='Indiquer les dates du contrôle fiscal.';

      deb:=e.DatExoDebut;
      fin:=e.DatExoFin;
            if not MyInputQuery_Date(self,'Date de début du contrôle fiscal','Indiquer la date de début du contrôle fiscal.',
                deb,'',[#0]) then exit;
            if not MyInputQuery_Date(self,'Date de fin du contrôle fiscal','Indiquer la date de fin du contrôle fiscal.',
                fin,'',[#0]) then exit;
      siren:=str_souschaine(siret,1,9);
      cheminFichier:='FEC';
      ExportEcriturePourControlFiscal(deb,fin,siren,cheminFichier,resultChoixParamFEC);
  end;
end;

procedure TMain.BtnProprietesSauvegardeAutoClick(Sender: TObject);
begin
 if (PgProperty = nil) then PgProperty:=TPgProperty.Create(self);
  PgProperty.PageControl1.TabIndex:=PgProperty.SauvegardeFermeture.PageIndex;
 GestListForm(PgProperty.Caption,Proprits1);
 AffichageForm(Monofenetrage,PgProperty);
 PgProperty.PageControl1.TabIndex:=PgProperty.SauvegardeFermeture.PageIndex;
end;

procedure TMain.ImportationdesrelevsEbics1Click(Sender: TObject);
begin
if  ImportationReleveEbics=nil then ImportationReleveEbics:=TImportationReleveEbics.Create(self);
    ImportationReleveEbics.Show;
end;

procedure TMain.Gestiondescomptesbancaires1Click(Sender: TObject);
begin
if E2_ComptesBancaires=nil then E2_ComptesBancaires:=TE2_ComptesBancaires.Create(self);
E2_ComptesBancaires.ShowModal;
end;

procedure TMain.Crationdesrglements1Click(Sender: TObject);
var
...
retourCompte:= AfficheCompteBanque(edit,sender = nil);
if(retourCompte.retour)then
begin
      datefin:=e.DatExoDebut;
      //ParamAfficheReleve.Titre:='';
      ParamAfficheReleve.Direct:=true;
      ParamAfficheReleve.DateFinReleve:=e.DatExoFin;

      ParamAfficheReleve.journal:=DMJournal.CompteDuJournal(retourCompte.journal);

      ParamAfficheReleve.libelleJournal:=DMJournal.LibelleDuJournal(retourCompte.journal);
      CreationReglementRelevesAffiche(ParamAfficheReleve);
end;
end;

procedure TMain.Balancedestiersetleuradresse1Click(Sender: TObject);
begin
EditionBalanceTiersL(C_Exportation,true,nil,nil,e.SautDePageAutomatique,true);
end;

procedure TMain.historiquedesequilibremouvement1Click(Sender: TObject);
begin
ShellExecute(Handle, 'open', PChar(E.RepertoireDossier+'DesequilibreBalanceMouvementListe.txt'), nil, nil, SW_SHOWNORMAL);
end;

procedure TMain.Importationcaissebis1Click(Sender: TObject);
var
etatTmp:TTypeEtatExercice;
begin
EtatTmp:=e.TypeEtatExercice;
e.TypeEtatExercice:=ET_Importation;
DMImport.Import_Piece_SurType(nil,true,'',C_IMPORT_CAISSEBis);
e.TypeEtatExercice:=EtatTmp;
end;

procedure TMain.ImportationCAISSE2Click(Sender: TObject);
var
etatTmp:TTypeEtatExercice;
begin
EtatTmp:=e.TypeEtatExercice;
e.TypeEtatExercice:=ET_Importation;
DMImport.Import_Piece_SurType(nil,true,DM_C_NomFichierImportCAISSEB,C_IMPORT_CAISSE_2);
e.TypeEtatExercice:=EtatTmp;
end;

procedure TMain.ImportationCAISSEClick(Sender: TObject);
begin
    GestListMenu(sender as TMenuItem);
end;
