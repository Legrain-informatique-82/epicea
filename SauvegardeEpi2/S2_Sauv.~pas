...
ListeFichiers,ListeFichiersDossier,ListeParamSauvegarde:TStringList;
...
//             Application.Terminate;
    ...
    //   showmessage(str_getstringelement(ListeParamSauvegarde.Strings[i],2,';'));
     ...
     begin
      ...
      if ParamCount > 3 then ListeRepertoireDEstination2.Add(ParamStr(3));//Lecteur:=
     end;
   end;
Lecteur :='a';
...
Jour:=libLocale.DateInfos(now).JourStr;
...
for i:=0 to ListeDesNomsdeDossier.Count-1 do
 begin
   ...
   NomFichierSauvegarde:=ListeDesNomsdeDossier.Strings[i]+Jour+Mois+DateInfos(now).AnStr+'-'+Heure+'.elz';
...
procedure TSauv.ZipMaster1Message(Sender: TObject; ErrCode: Integer;
  Message: String);
begin
   try
...
procedure TSauv.Button1KeyPress(Sender: TObject; var Key: Char);
begin
ShowMessage('Button1KeyPress');
end;

procedure TSauv.Button1DragDrop(Sender, Source: TObject; X, Y: Integer);
begin
ShowMessage('Button1DragDrop');
end;
